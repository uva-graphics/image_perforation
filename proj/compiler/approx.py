...
flags = compiler_config.compiler_options #'-O3 -fopenmp -funroll-loops -fmax-errors=3 -std=c++0x -fabi-version=6 -DBUILD_DEBUG=0 -I /opt/X11/include'
...
solver_compilation_statements = ''.join([ 'echo "if test '+f[0]+' -nt '+f[1]+'; then ' + compiler_config.cpp_compiler + ' -c '+flags+' '+f[0]+' -o '+f[1]+'; fi; " | bash ; ' for f in zip(solver_files,object_files) ])
cpp_compiler = solver_compilation_statements + compiler_config.cpp_compiler + ' '+object_files_string+' -o %s '+flags+' %s       ' + compiler_config.linker_options
